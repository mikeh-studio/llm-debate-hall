...
| 2026-07-15 | Game motion allowed on Pixel Stage only | User-approved gamification (debate actions, reactions, winner celebration). Idle sway, emote pops, and victory bounce are scoped to `.pixel-stage`; Classic view keeps minimal-functional motion. All stage animations respect `prefers-reduced-motion`. |
| 2026-07-15 | Action-token pips in brass | Each debater's limited move budget renders as small rotated-square pips (filled brass = available, hollow = spent) on participant chips, stage labels, and move cards. Move cards in the transcript use accent-bordered blocks; tribunal red remains judge-only. |
